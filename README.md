...
- Модель выработки по каждой турбине: эмпирическая кривая мощности по прогнозной скорости ветра на 100 м плюс градиентный бустинг на признаках прогноза (ветер 100 м и 10 м, порывы, направление, плотность воздуха, календарь, лид). Вес смеси подбирается на честной валидации.
...
Ожидаемый результат:
- `python -m app.cli train` печатает для каждой турбины вес смеси и ошибку на январе 2026: nMAE около 14.7% на горизонте 24 ч и около 16.8% на 48 ч (`docs/metrics.json`).
...
- `python -m app.cli backtest --no-llm` создаёт 28 файлов выпусков, `outputs/forecast_feb2026.csv` на 672 часа и `outputs/backtest_summary.csv`, где видно, в какие дни агент делал пересчёт.
...
Формат итогового файла `outputs/forecast_feb2026.csv`: `ts` (местное время Астаны), `turbine_1_lead1`, `turbine_2_lead1` (прогноз, выпущенный за сутки), `turbine_1_lead2`, `turbine_2_lead2` (за двое суток), `farm_mean_lead1`. Значения 0..1, как в исходных данных. Полная таблица всех выпусков: `outputs/forecast_all_issues.csv` (issue_date, ts, horizon_hour, lead_day, turbine, прогноз смеси, GBM и кривой отдельно).
