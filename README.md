...
- CLI, HTTP-сервис (FastAPI) для интеграции с системами диспетчера, тесты, контрольный скрипт для жюри, кэш всех внешних запросов для запуска без сети.
...
Docker (контрольный сценарий, затем HTTP-сервис на порту 8000):
```bash
...
python -m app.serve                            # HTTP-сервис: GET /health, /forecast/2026-02-10?horizon=48, /submission
```
