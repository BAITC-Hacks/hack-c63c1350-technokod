...
Полный контрольный сценарий одной командой: `bash scripts/demo.sh` (Linux/macOS/Git Bash) или `powershell scripts/demo.ps1` (Windows). Скрипт сам выбирает интерпретатор (`PYTHON`, затем `.venv`, затем `python` из PATH), останавливается на первой ошибке и выполняет семь шагов: проверка окружения → обучение (пропускается, если модели уже есть) → выпуск прогноза → бэктест февраля → сводка → автотесты → живая LLM-оркестрация (пропускается, если ключ не задан). Замерено: 20 секунд на обученных моделях и 38 секунд на чистом клоне с GitHub вместе с обучением. Сеть не требуется: архивные прогнозы лежат в `data/cache/weather/` (69 файлов, 4.9 МБ).

На свежем клоне первый прогон сценария занимает 38 секунд, из них 14 секунд — обучение (отдельно от этого установка зависимостей `pip install` идёт 1–2 минуты): файлы `models/*.joblib` не коммитятся, потому что pickle привязан к версии scikit-learn и на другой машине не загрузится. Если модели уже обучены, шаг 2 пропускается и сценарий проходит примерно за минуту.
